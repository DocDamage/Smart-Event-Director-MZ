...
def _color(step):
    return {"dialogue":"lightblue","narration":"lightblue","choice":"lightgreen","jump":"lightyellow","label":"lightyellow","condition":"lightsalmon","camera":"lightcyan","audio":"lightcyan","weather":"lightcyan","fadeOut":"lightcyan","fadeIn":"lightcyan","wait":"lightcyan","titleCard":"lightcyan","transition":"lightcyan","startQuest":"lightpink","moveRoute":"lightgray","lockPlayer":"lightgray","unlockPlayer":"lightgray"}.get(step.get("type",""),"white")
...
def main():
    ...
    if not path.exists():
        print(f"Error: File not found: {a.input_file}", file=sys.stderr); sys.exit(1)
    try:
        with open(path, "r", encoding="utf-8") as f:
            data = json.load(f)
    except json.JSONDecodeError as e:
        print(f"Error: Invalid JSON in {a.input_file}: {e}", file=sys.stderr); sys.exit(1)
    steps = data.get("steps", [])
    if not steps:
        print("Warning: No steps found in scene file.", file=sys.stderr)
    if a.png:
        try:
            subprocess.run(["dot","-Tpng","-o",a.png], input=generate_dot(steps), text=True, capture_output=True, check=True)
        except FileNotFoundError:
            print("Error: Graphviz 'dot' command not found. Install the system package (https://graphviz.org/download/) or ensure it is on PATH.", file=sys.stderr)
            sys.exit(1)
        except subprocess.CalledProcessError as e:
            print(f"Error: Graphviz failed: {e.stderr}", file=sys.stderr); sys.exit(1)
